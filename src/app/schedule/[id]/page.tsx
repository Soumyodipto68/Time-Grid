...
type Schedule = { id: string; name: string; members: TeamMember[] };
async function getSchedule(id: string): Promise<Schedule | null> {
  const baseUrl = process.env.NEXT_PUBLIC_APP_URL || "http://localhost:3000";
  const response = await fetch(`${baseUrl}/api/shared-schedules/${id}`, {
    cache: "no-store",
  });
  if (!response.ok) {
    return null;
    ...
    <main className="min-h-screen bg-[#070b12] px-8 py-10 text-white">
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mb-10">
          ...
          <p className="mt-2 text-gray-400"> Shared team schedule </p>{" "}
        </div>{" "}
          ...
          {schedule.members.map((member) => (
            ...
            >
              ...
              <div className="flex items-center gap-3">
                {" "}
                <span className="text-3xl"> {member.country} </span>{" "}
                <div>
                  {" "}
                  <h2 className="font-semibold"> {member.name} </h2>{" "}
                  <p className="text-sm text-gray-400"> {member.city} </p>{" "}
                </div>{" "}
              </div>{" "}
              <div className="mt-6">
                {" "}
